...
def headline_tone_score(title: str) -> float:
    lowered = title.lower()
    ...
    phrase_adjustment = sum(weight for phrase, weight in phrase_weights.items() if phrase in lowered)
    raw = positive - negative
    lexical_score = max(min((raw + phrase_adjustment) / 4.0, 1.0), -1.0)
    semantic_score = price_phrase_tone_score(title)
...
def price_phrase_tone_score(title: str) -> float:
    lowered = title.lower()
    score = 0.0
    ...
    words = re.findall(r"[A-Za-z']+", lowered)
    if any(term in lowered for term in PRICE_CONTEXT_TERMS):
        for word in words:
    ...
    if "price" in lowered or "prices" in lowered:
        if any(token in lowered for token in ("record high", "all-time high", "multi-year high")):
            score = max(score, 0.82)
        if any(token in lowered for token in ("record low", "multi-year low")):
            score = min(score, -0.82)

    return max(min(score, 1.0), -1.0)


def price_impact_rule_score(title: str) -> float:
    lowered = title.lower()
    words = re.findall(r"[A-Za-z']+", lowered)
    if not words:
        return 0.0
    ...
    phrase_adjustment = sum(weight for phrase, weight in PRICE_BULLISH_PHRASES.items() if phrase in lowered)
    phrase_adjustment += sum(weight for phrase, weight in PRICE_BEARISH_PHRASES.items() if phrase in lowered)
    if "price" in lowered or "prices" in lowered:
        if any(token in lowered for token in ("rise", "rises", "higher", "hike", "soar", "surge", "jump")):
            phrase_adjustment += 0.8
        if any(token in lowered for token in ("fall", "falls", "lower", "ease", "slump", "drop")):
            phrase_adjustment -= 0.8
    raw = bullish - bearish
    return max(min((raw / 5.0) + (phrase_adjustment / 3.0), 1.0), -1.0)
...
def combined_headline_signal(title: str, commodity: dict[str, Any]) -> dict[str, Any]:
    phrase_tone_score = price_phrase_tone_score(title)
    lexical_tone_score = headline_tone_score(title)
    ...
    rule_score = price_impact_rule_score(title)
