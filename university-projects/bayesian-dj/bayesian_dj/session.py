    ...
    def recommend_next(
        ...
        if best_choice is None:
            if len(available_idx) == 0:
                return None
            fallback_local = int(candidate_order[0]) if len(candidate_order) else 0
            fallback_pool_idx = int(available_idx[fallback_local])
            fallback_song = self.pool.get_song_info(fallback_pool_idx)
            best_choice = (fallback_pool_idx, fallback_local, fallback_song)
            best_score = float(scores[fallback_local]) if len(scores) > fallback_local else 0.0
