...
def fetch_spotify_song_pool(
    ...
    """Build a bounded Spotify candidate pool without brute-force search."""
    intent = intent or PromptIntent()
    profile = current_taste_profile()
    saved_track_ids: set[str] = set(profile.get("spotify_saved_track_ids", []))
    recent_track_ids: set[str] = set(profile.get("spotify_recent_track_ids", []))
    top_track_ids: set[str] = set(profile.get("spotify_top_track_ids", []))
    spotify_summary = profile.get("spotify_user_summary") or {}
    top_artist_names: list[str] = list(spotify_summary.get("artists") or top_affinity_items(profile.get("artist_affinity", {}), limit=4))
    top_genres: list[str] = list(spotify_summary.get("genres") or top_affinity_items(profile.get("genre_affinity", {}), limit=4))
    liked_payloads: list[dict[str, Any]] = list(profile.get("liked_songs", []))
    top_all_time_ids: list[str] = list(top_track_ids)

    prompt_label = ", ".join(spec.seed_artists or intent.seed_artists or spec.genres or spec.moods or spec.spotify_search_queries[:1] or ["open request"])
    print(f"Prompt: {prompt_label}")

    tracks: dict[str, dict[str, Any]] = {}
    stage_calls = 0
    max_stage_calls = 6
    stage_pool_target = min(max(25, n_target // 4), 30)
    ...
    spotify_genre_map: dict[str, str] = {
        ...
        "alternative": "alternative", "indie": "indie", "rock": "rock", "funk": "funk",
        "disco": "disco", "house": "house", "electronic": "electronic", "ambient": "ambient",
        "jazz": "jazz", "blues": "blues", "country": "country", "folk": "folk", "acoustic": "acoustic",
    }
    ...
    def _track_prompt_score(track: dict[str, Any], query: str = "", genre_tag: str = "") -> float:
        ...
        if intent.seed_tracks and any(track_ref.lower() in track_name for track_ref in intent.seed_tracks[:2]):
            score += 0.55
        if genre_tag and normalize_affinity_label(genre_tag) in {
            normalize_affinity_label(item) for item in (intent.genres or spec.genres or [])
        }:
            score += 0.30
        for term in intent.semantic_terms[:4]:
            term_lower = term.lower()
            if term_lower in track_name or term_lower in artist_names:
                score += 0.12
    ...
    def _ingest_track_obj(
        ...
        *,
        ...
        novelty_score = 0.82 + 0.22 * intent.exploration_weight
        if is_saved:
            novelty_score -= 0.22
        if is_recent:
            novelty_score -= 0.18
        if is_top_track:
            novelty_score -= 0.14
        if source_type == "related_artist_top":
            novelty_score += 0.12
        elif source_type == "recommendation":
            novelty_score += 0.08
        elif source_type == "adjacent_search":
            novelty_score += 0.12
        elif source_type == "history_anchor":
            novelty_score -= 0.08
        tracks[tid] = {
            "track_id": tid,
    ...
    def _stage_get(stage_name: str, url: str, params: dict[str, str] | None = None) -> dict[str, Any] | None:
        nonlocal stage_calls
        if stage_calls >= max_stage_calls:
            print(f"Stage: {stage_name}")
            print("Tracks returned: 0")
            print(f"Candidate pool: {len(tracks)}")
            print("Selected track: pending")
            return None
        if stage_calls > 0:
            time.sleep(0.12)
        stage_calls += 1
        payload = spotify_api_get(url, token, params)
        returned = 0
        if isinstance(payload, dict):
            if isinstance(payload.get("tracks"), dict):
                returned = len((payload.get("tracks") or {}).get("items", []) or [])
            elif isinstance(payload.get("tracks"), list):
                returned = len(payload.get("tracks") or [])
            elif isinstance(payload.get("artists"), dict):
                returned = len((payload.get("artists") or {}).get("items", []) or [])
            elif isinstance(payload.get("artists"), list):
                returned = len(payload.get("artists") or [])
        print(f"Stage: {stage_name}")
        print(f"Tracks returned: {returned}")
        print(f"Candidate pool: {len(tracks)}")
        print("Selected track: pending")
        return payload if isinstance(payload, dict) else None

    def _should_stop() -> bool:
        return len(tracks) >= stage_pool_target or stage_calls >= max_stage_calls

    def _resolve_primary_artist_name() -> str:
        if intent.seed_artists:
            return str(intent.seed_artists[0])
        if spec.seed_artists:
            return str(spec.seed_artists[0])
        query = str(spec.spotify_search_queries[0] if spec.spotify_search_queries else "").strip()
        lowered = normalize_affinity_label(query)
        cleaned = re.sub(r"\b(play|me|some|please|give)\b", " ", lowered)
        cleaned = re.sub(r"\b(vibes|music|songs|song|tracks|track)\b", " ", cleaned)
        cleaned = re.sub(r"\s+", " ", cleaned).strip()
        return cleaned.title() if cleaned else ""

    def _resolve_artist(query: str) -> dict[str, Any] | None:
        if not query:
            return None
        payload = _stage_get(
            "artist_resolve",
            "https://api.spotify.com/v1/search",
            {"q": query, "type": "artist", "limit": "1", "market": "US"},
        )
        items = (payload or {}).get("artists", {}).get("items", []) or []
        return items[0] if items else None

    def _search_tracks(stage_name: str, query: str, genre_tag: str = "", source_type: str = "search") -> None:
        if not query or _should_stop():
            return
        payload = _stage_get(
            stage_name,
            "https://api.spotify.com/v1/search",
            {"q": query, "type": "track", "limit": str(max_results_per_query), "market": "US"},
        )
        for track in (payload or {}).get("tracks", {}).get("items", []) or []:
            _ingest_track_obj(track, genre_tag=genre_tag, source_type=source_type, prompt_query=query)

    def _fetch_artist_top_tracks(stage_name: str, artist_id: str, genre_tag: str = "", source_type: str = "seed_artist_top") -> None:
        if not artist_id or _should_stop():
            return
        payload = _stage_get(
            stage_name,
        ...
        for track in ((payload or {}).get("tracks") or [])[:max_results_per_query]:
            _ingest_track_obj(track, genre_tag=genre_tag, source_type=source_type, prompt_query=genre_tag)

    def _fetch_recommendations(stage_name: str, artist_id: str = "", genre_slug: str = "") -> None:
        if _should_stop():
            return
        params: dict[str, str] = {"limit": str(max_results_per_query), "market": "US"}
        if artist_id:
            params["seed_artists"] = artist_id
        elif genre_slug:
            params["seed_genres"] = genre_slug
        else:
            return
        payload = _stage_get("recommendation_" + stage_name, "https://api.spotify.com/v1/recommendations", params)
        for track in ((payload or {}).get("tracks") or [])[:max_results_per_query]:
            _ingest_track_obj(track, genre_tag=genre_slug.replace("-", " "), source_type="recommendation", prompt_query=genre_slug)

    def _fetch_related_artist_name(artist_id: str) -> str:
        if not artist_id or _should_stop():
            return ""
        payload = _stage_get(
            "related_artists",
        ...
        artists = ((payload or {}).get("artists") or [])[:3]
        return str(artists[0].get("name", "") or "") if artists else ""

    primary_artist = _resolve_primary_artist_name()
    primary_genre = str((intent.genres or spec.genres or top_genres or [""])[0] or "")
    primary_genre_slug = spotify_genre_map.get(primary_genre.lower().replace("-", " "), primary_genre.lower().replace(" ", "-")) if primary_genre else ""
    primary_query = primary_artist or spec.seed_track or (spec.spotify_search_queries[0] if spec.spotify_search_queries else "")
    if not primary_query:
        primary_query = " ".join((intent.semantic_terms or spec.moods or top_genres or ["music"])[:3]).strip()

    _search_tracks("strict_prompt_match", primary_query, primary_genre, "search")

    if not _should_stop():
        artist_payload = _resolve_artist(primary_artist or primary_query)
        if artist_payload:
            resolved_artist_id = str(artist_payload.get("id", "") or "")
            primary_artist = str(artist_payload.get("name", "") or primary_artist)

    if not _should_stop() and resolved_artist_id:
        _fetch_artist_top_tracks("artist_top_tracks", resolved_artist_id, primary_genre, "seed_artist_top")

    if not _should_stop() and (resolved_artist_id or primary_genre_slug):
        _fetch_recommendations("seed_context", resolved_artist_id, primary_genre_slug)

    if not _should_stop() and resolved_artist_id:
        related_artist_name = _fetch_related_artist_name(resolved_artist_id)

    if not _should_stop():
        vibe_terms = list(dict.fromkeys((intent.moods or spec.moods or []) + intent.production_tags[:2] + intent.activity_context[:1]))
        genre_vibe_query = " ".join(([primary_genre] if primary_genre else []) + vibe_terms[:3]).strip()
        if genre_vibe_query:
            _search_tracks("genre_vibe_search", genre_vibe_query, primary_genre, "adjacent_search")

    if not _should_stop():
        fallback_artist = related_artist_name or (top_artist_names[0] if top_artist_names else "")
        fallback_query = " ".join(part for part in [fallback_artist, primary_genre, *(intent.moods[:2] if intent.moods else [])] if part).strip()
        if fallback_query:
            _search_tracks("fallback_seed_artist", fallback_query, primary_genre, "adjacent_search")

    if not tracks:
        for liked in liked_payloads[:8]:
            track_id = str(liked.get("track_id", "") or "")
            if not track_id:
                continue
            tracks[track_id] = {
                "track_id": track_id,
                ...
                "track_genre": liked.get("genre", primary_genre),
                "popularity": 50,
                "artist_popularity": 0,
                "release_year": 0,
                "source_type": "history_anchor",
                "prompt_score": 0.35,
                "is_saved": track_id in saved_track_ids,
                "is_recent": track_id in recent_track_ids,
                "is_top_track": track_id in top_track_ids,
                "novelty_score": 0.22,
            }
    ...
    if unique_artist_ids:
        payload = spotify_api_get("https://api.spotify.com/v1/artists", token, {"ids": ",".join(unique_artist_ids[:50])})
        for artist in ((payload or {}).get("artists") or []):
            if isinstance(artist, dict) and artist.get("id"):
                artist_meta[str(artist["id"])] = artist

    track_ids = list(tracks.keys())[:100]
    audio_features: dict[str, dict[str, Any]] = {}
    if track_ids:
        payload = spotify_api_get("https://api.spotify.com/v1/audio-features", token, {"ids": ",".join(track_ids)})
        for feat in ((payload or {}).get("audio_features") or []):
            if feat and feat.get("id"):
                audio_features[str(feat["id"])] = feat
    ...
    for tid, track_data in tracks.items():
        ...
        for feature_name in AUDIO_FEATURES:
            if feat and feat.get(feature_name) is not None:
                row[feature_name] = float(feat.get(feature_name, 0.0))
            else:
                row[feature_name] = float(feature_defaults[feature_name])
        row["_seed_all_time_rank"] = top_all_time_ids.index(tid) if tid in top_all_time_ids else -1
        rows.append(row)

    selected_debug = rows[0]["track_name"] if rows else "none"
    print(f"Selected track: {selected_debug}")
